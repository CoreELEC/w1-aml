...
void wf5g_txiqdc(struct wf5g_txdciq_result* wf5g_txdciq_result, U8 tx_pa_gain, U8 tx_mxr_gain)
{
    ...
    if(!RG_TX_A2_saved) {
        RG_TX_A2_saved = True;
        rg_tx_a2_i.b.RG_WF5G_IQCAL_EN = rg_tx_a2.b.RG_WF5G_IQCAL_EN;
    }
    rg_tx_a2.b.RG_WF5G_IQCAL_EN = 0x1;
    ...
    rg_rx_a2.b.RG_WF_RX_GAIN_MAN = 0x1a;
    ...
    rg_esti_a21.b.rg_tx_dcmax_i = 0x7f;
    rg_esti_a21.b.rg_tx_dcmax_q = 0x7f;
    ...
    rg_esti_a22.b.rg_tx_dcmin_i = 0x80;
    rg_esti_a22.b.rg_tx_dcmin_q = 0x80;
    ...
    rg_esti_a23.b.rg_tx_dc_i_step = 0x1;
    rg_esti_a23.b.rg_tx_dc_q_step = 0x1;
    rg_esti_a23.b.rg_tx_alpha_step = 0x1;
    rg_esti_a23.b.rg_tx_theta_step = 0x1;
    rg_esti_a23.b.rg_txirr_oper_bnd = 0x4;
    ...
    while(unlock_cnt < TIME_OUT_CNT)
    {
        if(dc_rdy == 0 || irr_rdy == 0)
        {
            ...
            st_printf("unlock_cnt:%d, TX DC IRR NOT DOING...\n", unlock_cnt);
        }
        else
        {
            PUTS("TX DC IRR READY\n");
            break;
        } //  if(dc_rdy == 0|| irr_rdy == 0)
    } // while(unlock_cnt < TIME_OUT_CNT)
...
//set TX DC/IRR comp manual mode to 1
    ...
    PUTS("----------------------> TX DC&IRR out\n");
    st_printf("dci: 0x%x dcq: 0x%x alpha:0x%x  theta:0x%x\n", dci, dcq, alpha, theta);
    ...
    // Revert the original value before calibration back
    rg_tx_a2.data = rf_read_register(RG_TX_A2);
    rg_tx_a2.b.RG_WF5G_IQCAL_EN = rg_tx_a2_i.b.RG_WF5G_IQCAL_EN;
    rf_write_register(RG_TX_A2, rg_tx_a2.data);
    // Revert the original value before calibration back
    rg_tx_a3.data = rf_read_register(RG_TX_A3);
