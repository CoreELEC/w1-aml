...
void t9026_wf2g_txdpd_start(U8 sync_gap, U8 bw_idx)
{
    ...
    if(bw_idx == 0){
        rg_xmit_a46.b.rg_txpwc_comp_man = 0x60;                        // i2c_set_reg_fragment(0xe4b8, 31, 24, 0xb0); //set pwtrl gain
        //fi_ahb_write(0x0000b22c, 0x80000000);
    }else if(bw_idx == 1){
        rg_xmit_a46.b.rg_txpwc_comp_man = 0x90;                        //0x50(dac gain:41);0x80(dac gain:21);0x90(dac gain:01)
    ...
    fi_ahb_write(PHY_TX_LPF_BYPASS, 0x1);  //open spectrum mask
    ...
    rg_esti_a66.b.rg_adda_esti_count = 0x186a00;  //0x30d400;   //0x7a1200;
