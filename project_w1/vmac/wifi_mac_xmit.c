    ...
    if (eh->ether_type == __constant_htons(ETHERTYPE_IP)) {
        if (iphdrp->protocol == IPPROTO_TCP) {
            ...
            if ((th->ack) && (!th->psh) && (!th->fin) && (!th->syn)) {
                if (!((unsigned long)(skb->data + skb->len) - (unsigned long)((unsigned char *)th + th->doff * 4)))
        ...
        src_ip = (unsigned char *)iphdrp + 14;
        for (i = 0; i < 4; ++i) {
            if (src_ip[i] != 0) {
                src_ip_valid = 1;
                break;
...
unsigned short wifi_mac_check_sum_calc(unsigned short len_udp, unsigned char * src_addr,
...
int wifi_mac_udp_csum(struct sk_buff *skb)
{
    ...
    uh->check = 0;
    uh->check = wifi_mac_check_sum_calc(__constant_htons(uh->len),(unsigned char *) &iphdrp->saddr,(unsigned char *)&iphdrp->daddr, 17, (unsigned char *)uh);
...
int wifi_mac_tcp_csum(struct sk_buff *skb)
{
    ...
    th->check = 0;
    th->check = wifi_mac_check_sum_calc(data_len, (unsigned char *) &iphdrp->saddr, (unsigned char *)&iphdrp->daddr, 6, (unsigned char *)th);
...
int wifi_mac_hardstart(struct sk_buff *skb, struct net_device *dev)
{
    ...
    if (NET80211_IF_RUN_UP(wifimac) == 0) {
        DPRINTF(AML_DEBUG_WARNING, "<WARNNING::> %s %d,netdev is not run up!! \n",__func__,__LINE__);
        goto bad;
    }

    if (wnet_vif->vm_state != WIFINET_S_CONNECTED) {
        DPRINTF(AML_DEBUG_XMIT, "WARNNING: %s no connection\n",__func__);
        goto bad;
    }
    ...
    if ((wnet_vif->vm_opmode == WIFINET_M_STA) && (wnet_vif->vm_flags_ext & WIFINET_C_STA_FORWARD)) {
        if (WIFINET_ADDR_EQ(eh->ether_shost, dev->dev_addr)) {
            if (eh->ether_type ==__constant_htons( ETHERTYPE_PAE)) {
                ...
                DPRINTF(AML_DEBUG_WARNING,"<WARNNING> %s %d \n",__func__,__LINE__);
                goto bad;
            }

        } else {
            DPRINTF(AML_DEBUG_WARNING,"WARNNING::skb not for this Dev,arrive here, why?? <running> %s %d \n",__func__,__LINE__);
            goto bad;
        }
    }
    ...
    if (sta == NULL) {
        DPRINTF(AML_DEBUG_WARNING,"<WARNING> %s not find sta, mac:%02x:%02x:%02x:%02x:%02x:%02x\n",
            __func__, eh->ether_dhost[0], eh->ether_dhost[1], eh->ether_dhost[2], eh->ether_dhost[3], eh->ether_dhost[4], eh->ether_dhost[5]);
        goto bad;
    }
    ...
    if (wifi_mac_classify(sta, skb)) {
        WIFINET_DPRINTF_STA(AML_DEBUG_XMIT, sta, "%s: discard, classification failure", __func__);
        goto bad;
    }
    ...
    if (sta->sta_channel_switch_mode == 1) {
        DPRINTF(AML_DEBUG_WARNING,"<WARNING> %s %d, channel_switch_mode[1]:drop\n",__func__,__LINE__);
        goto bad;
    }
...
void wifi_mac_send_setup(struct wlan_net_vif *wnet_vif,
                        ...
                        int type,
                        const unsigned char sa[WIFINET_ADDR_LEN],
                        const unsigned char da[WIFINET_ADDR_LEN],
                        const unsigned char bssid[WIFINET_ADDR_LEN])
{
...
wifi_mac_mgmt_output(struct wifi_station *sta, struct sk_buff *skb, int type)
{
    ...
    wifi_mac_send_setup(wnet_vif, sta, wh,
                       WIFINET_FC0_TYPE_MGT | type,
                       wnet_vif->vm_myaddr, sta->sta_macaddr, sta->sta_bssid);

    if ((cb->flags & M_LINK0) != 0 && sta->sta_challenge != NULL)
    {
        cb->flags &= ~M_LINK0;
        WIFINET_DPRINTF_MACADDR( AML_DEBUG_CONNECT, wh->i_addr1,
    ...
    struct sk_buff *skb, unsigned char *macaddr)
{
    ...
    wh = (struct wifi_frame *)os_skb_push(skb, sizeof(struct wifi_frame));
#ifdef CONFIG_P2P
    if ((wnet_vif->vm_p2p->p2p_role == NET80211_P2P_ROLE_DEVICE) 
        || (wnet_vif->vm_p2p->p2p_role == NET80211_P2P_ROLE_CLIENT))
    {
    ...
    wifi_mac_send_setup(wnet_vif, sta, wh,
                       WIFINET_FC0_TYPE_MGT | WIFINET_FC0_SUBTYPE_PROBE_RESP,
                       sa,macaddr, bssid);
        ...
        unsigned char pwr_flag, unsigned char qos, int ac)
{
    ...
    {
        null_data.bw = (sta->sta_chbw == WIFINET_BWC_WIDTH20)
    ...
    struct wifi_mac_key *key, struct sk_buff *skb, int ismulticast)
{
    ...
    {
        pskb = skb_copy_expand(skb, need_headroom+32, need_tailroom+32 , GFP_ATOMIC);
        if (pskb == NULL)
        {
...
struct sk_buff *wifi_mac_encap(struct wifi_station *sta, struct sk_buff *skb)
{
    ...
    if (addqos)
    {
        struct wifi_qos_frame *qwh =  (struct wifi_qos_frame *) wh;
        unsigned char *qos;
        int tid;

        qos = &qwh->i_qos[0];
        if (use4addr)
            qos += WIFINET_ADDR_LEN;
        tid = cb->u_tid;
...
{
    memcpy(frm, (unsigned char *)&wifimac->wm_countryinfo,
           wifimac->wm_countryinfo.country_len + 2);
...
wifi_mac_setup_wpa_ie(struct wlan_net_vif *wnet_vif, unsigned char *ie)
{
    ...
    *frm++ = 0;
    ...
    KASSERT(ie[1]+2 <= sizeof(struct wifi_mac_ie_wpa),
            ("WPA IE too big, %u > %zu",
             ie[1]+2, sizeof(struct wifi_mac_ie_wpa)));
    return frm;
}
...
unsigned char *
wifi_mac_add_extcap(unsigned char *frm,struct wifi_station *sta)
{
...
wifi_mac_add_htcap(unsigned char *frm, struct wifi_station *sta)
{
    ...
    if(wifimac->wm_flags & WIFINET_F_LDPC)
    {
        ...
        //printk("wifi_mac_add_htcap ht support ldpc\n");
    }
...
wifi_mac_add_vht_cap(unsigned char *frm, struct wifi_station *sta)
{
    ...
    if ((wifimac->wm_flags_ext & WIFINET_FEXT_SHORTGI_ENABLE)) //lg fix
    {
...
#ifdef SU_BF
    if (wnet_vif->vm_vhtcap & WIFINET_VHTCAP_SU_BFMEE)
    {
...
#ifdef MU_BF
    if (wnet_vif->vm_vhtcap & WIFINET_VHTCAP_MU_BFMEE)
    {
        if (wnet_vif->vm_opmode == WIFINET_M_HOSTAP)
    ...
    if ((wnet_vif->vm_vhtcap & WIFINET_VHTCAP_SU_BFMEE) || (wnet_vif->vm_vhtcap & WIFINET_VHTCAP_MU_BFMEE))
    ...
    if(wnet_vif->vm_vhtcap & WIFINET_VHTCAP_MAX_MPDU_LEN_11454)
    {
        ...
        //printk("<running> %s (%d) support WIFINET_VHTCAP_MAX_MPDU_LEN_11454\n" ,__func__,__LINE__);
    }else if(wnet_vif->vm_vhtcap & WIFINET_VHTCAP_MAX_MPDU_LEN_7991)
    {
        SET_VHT_CAP_MAX_MPDU(vht_cap_ie->vht_cap_info,   VHT_CAP_MAX_MPDU_LEN_7991);
        //printk("<running> %s (%d)  support VHT_CAP_MAX_MPDU_LEN_7991\n" ,__func__,__LINE__);
    }else
    {
        SET_VHT_CAP_MAX_MPDU(vht_cap_ie->vht_cap_info,   VHT_CAP_MAX_MPDU_LEN_3895);
        //printk("<running> %s (%d) support VHT_CAP_MAX_MPDU_LEN_3895\n" ,__func__,__LINE__);
    }
...
unsigned char*
wifi_mac_add_vht_opt(unsigned char *frm,
    struct wifi_station *sta, unsigned int sub_type)
{
   ...
   //default xsect_bw should be 20M.
    ...
    {
        printk("%s(%d) Not supported sub_type:0x%x\n ", __func__, __LINE__,sub_type);
    }
    ...
    if (sta->sta_wnet_vif->vm_curchan != NULL) {
        ...
        printk("%s(%d) vm_curchan is NULL!!!, vm_macmode:%d\n", __func__, __LINE__, sta->sta_wnet_vif->vm_mac_mode);
   ...
   // vhtop->vht_op_ch_freq_seg2 = vmac->iv_bsschan->ic_vhtop_ch_freq_seg2;   //Not  implemented yet
    ...
    DPRINTF(AML_DEBUG_BWC,"%s(%d) sub_type:0x%x\n self_bw: 0x%x\n peer_bw: 0x%x\n xsect_bw:0x%x\n bw_in_frm: 0x%x\n vht_op_ch_freq_seg1:%d\n vht_op_ch_freq_seg2:%d\n",
				__func__, __LINE__,
				sub_type, 
				sta->sta_wnet_vif->vm_bandwidth,
				sta->sta_chbw, 
				xsect_bw,
    ...
    const unsigned char *ssid, size_t ssidlen, const void *optie, size_t optielen)
{
    ...
    vm_StaAtomicInc(sta, __func__);
    skb = wifi_mac_get_mgmt_frm(wifimac, &frm, skb_len);
    if (skb == NULL) {
        wnet_vif->vif_sts.sts_tx_no_buf++;
        wifi_mac_FreeNsta(sta);
        return ENOMEM;
                                                        ...
                                                        struct wifi_station *sta,void *arg)
{
    ...
    skb = wifi_mac_get_mgmt_frm(wifimac, &frm,
                              8
                              ...
                              + 3
                              + (wnet_vif->vm_flags & WIFINET_F_WPA ?
                                 2*sizeof(struct wifi_mac_ie_wpa) : 0)
                              + sizeof(struct wifi_mac_wme_param)
                              +  sizeof(struct wifi_mac_ie_htcap)
                              + sizeof(struct wifi_mac_ie_htinfo)
                              + sizeof (struct wifi_mac_ie_obss_scan)
                              + sizeof (struct wifi_mac_ie_ext_cap)
                              + WIFINET_APPIE_MAX
                            + sizeof(struct wifi_mac_ie_vht_cap)
                            + sizeof(struct wifi_mac_ie_vht_opt)
                            + sizeof(struct wifi_mac_ie_vht_txpwr_env)
                            + sizeof(struct wifi_mac_ie_vht_ch_sw_wrp) 
                            /*The following capability does not support*/
                            //+ sizeof(struct wifi_mac_ie_vht_ext_bss_ld)
                            //+ sizeof(struct wifi_mac_ie_vht_quiet_chn)
                            //+ sizeof(struct wifi_mac_ie_vht_opt_md_ntf)
                             );
    ...
    *(unsigned short *)frm = htole16(wnet_vif->vm_bcn_intval);
    ...
    //p2p, 0
#ifdef CONFIG_P2P
    if ((sta->sta_flags_ext & WIFINET_NODE_P2P_REQ)
        || ((wnet_vif->vm_p2p->p2p_enable)
        && ((wnet_vif->vm_p2p->p2p_role== NET80211_P2P_ROLE_DEVICE)
        || (wnet_vif->vm_p2p->p2p_role == NET80211_P2P_ROLE_CLIENT))))
    {
        capinfo = 0;
    }
    ...
    *(unsigned short *)frm = htole16(capinfo);
    ...
    //p2p wild scan, ssid=P2P_WILDCARD_SSID
    //go/ap, sta_essid
    if (sta->sta_flags_ext & WIFINET_NODE_P2P_REQ)
    {
    ...
    if (wnet_vif->vm_p2p->p2p_enable && (sta->sta_p2p_ie[0] != NULL))
    {
        ...
        {
            //printk("add p2p ie for proberesp, len=%d, to mac=%s\n",
            //      wnet_vif->vm_p2p->p2p_app_ie[WIFINET_APPIE_FRAME_PROBE_RESP].length, ether_sprintf(arg));
...
int wifi_mac_send_auth(struct wlan_net_vif *wnet_vif,struct wifi_station *sta,void *arg)
{
    ...
    *(int *)arg &= 0xffff;
    ...
    is_shared_key = has_challenge || *(int *)arg >= WIFINET_AUTH_SHARED_RESPONSE ||
        ...
        ((wnet_vif->vm_mainsta->sta_authmode == WIFINET_AUTH_AUTO) && (wnet_vif->vm_cur_authmode == WIFINET_AUTH_SHARED))));

    skb = wifi_mac_get_mgmt_frm(wnet_vif->vm_wmac, &frm, 3 * sizeof(unsigned short)
    ...
    ((unsigned short *)frm)[0] =(is_shared_key) ? htole16(WIFINET_AUTH_ALG_SHARED) : htole16(WIFINET_AUTH_ALG_OPEN);
...
int wifi_mac_send_deauth(struct wlan_net_vif  *wnet_vif, struct wifi_station *sta,void *arg)
{
    ...
    WIFINET_DPRINTF_STA(AML_DEBUG_CONNECT, sta, "send station deauthenticate (reason %d)", *(int *)arg);

    skb = wifi_mac_get_mgmt_frm(wnet_vif->vm_wmac, &frm, sizeof(unsigned short));
    if (skb == NULL)
    {
        WIFINET_DPRINTF_STA( AML_DEBUG_CONNECT, sta, "%s: DeAuth: Unable to allocate mgmt frame", __func__);
        return ENOMEM;
    }
...
int wifi_mac_send_assoc_req(struct wlan_net_vif *wnet_vif,
                                                    struct wifi_station *sta,int type)
{
    struct wifi_mac *wifimac = wnet_vif->vm_wmac;
    struct sk_buff *skb;
    unsigned char *frm;
    ...
    skb = wifi_mac_get_mgmt_frm(wifimac, &frm,
                              ...
                              + 2 + (WIFINET_RATE_MAXSIZE - WIFINET_RATE_SIZE)
                              + 4
                              + 2 + WIFINET_SUPPCHAN_LEN
                              + sizeof(struct wifi_mac_ie_wme)
                              + sizeof(struct wifi_mac_ie_htcap)
                              + (wnet_vif->vm_opt_ie_len)
    ...
    if ((wifimac->wm_flags & WIFINET_F_SHPREAMBLE) &&
        WIFINET_IS_CHAN_2GHZ(wifimac->wm_curchan))
        capinfo |= WIFINET_CAPINFO_SHORT_PREAMBLE;
    if ((sta->sta_capinfo & WIFINET_CAPINFO_SHORT_SLOTTIME) &&
        (wifimac->wm_caps & WIFINET_C_SHSLOT))
        capinfo |= WIFINET_CAPINFO_SHORT_SLOTTIME;
    ...
    *(unsigned short *)frm = sta->sta_listen_intval;//htole16(wnet_if->vm_lintval);
    ...
    //p2p assocreq ie from connect cmd
    ...
    if (wnet_vif->vm_p2p->p2p_enable
        ...
        && wnet_vif->vm_p2p->wfd_app_ie[WIFINET_APPIE_FRAME_ASSOC_REQ].ie)
    {
    ...
    *(unsigned short *)frm = htole16(*(int *)arg);
    ...
    if (*(int *)arg == WIFINET_STATUS_SUCCESS)
    {
        *(unsigned short *)frm = htole16(sta->sta_associd);
        ...
        wifimac->drv_priv->drv_ops.RegisterStationID(wifimac->drv_priv,wnet_vif->wnet_vif_id,sta->sta_associd,sta->sta_macaddr, sta->sta_encrypt_flag);
    }
    else
    ...
    {
        DPRINTF(AML_DEBUG_P2P, "%s %d add p2p ie for assocresp\n",__func__,__LINE__);
...
int wifi_mac_send_disassoc(struct wlan_net_vif *wnet_vif, struct wifi_station *sta, void *arg)
{
    ...
    WIFINET_DPRINTF_STA(AML_DEBUG_CONNECT, sta, "send station disassociate (reason %d)", *(int *)arg);
    skb = wifi_mac_get_mgmt_frm(wnet_vif->vm_wmac, &frm, sizeof(unsigned short));
    if (skb == NULL)
    {
        WIFINET_DPRINTF_STA( AML_DEBUG_CONNECT, sta,
                             "%s: disassoc: Unable to allocate mgmt frame", __func__);
        return ENOMEM;
    }
        ...
        case WIFINET_ACTION_CAT_BA:
            ...
            switch (action)
            {
                case WIFINET_ACTION_BA_ADDBA_REQUEST:
                    ...
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_ba_addbarequest));
                    if (skb == NULL)
                    {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    ...
                    addbarequest->rq_header.ia_category = WIFINET_ACTION_CAT_BA;
                ...
                case WIFINET_ACTION_BA_ADDBA_RESPONSE:
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_ba_addbaresponse));
                    if (skb == NULL)
                    {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    ...
                    addbaresponse->rs_header.ia_category    = WIFINET_ACTION_CAT_BA;
                    addbaresponse->rs_header.ia_action      = action;
                    wifi_mac_addba_rsp_setup(sta, tid_index, &addbaresponse->rs_dialogtoken, &statuscode, &baparamset, &batimeout);
                    *(unsigned short *)&addbaresponse->rs_baparamset = htole16(*(unsigned short *)&baparamset);
                    ...
                    DPRINTF(AML_DEBUG_ADDBA, "%s: ADDBA response action mgt frame. TID %d, buffer size %d, status %d \n",
                        __func__, tid_index, baparamset.buffersize, statuscode);
                    break;

                case WIFINET_ACTION_BA_DELBA:
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_ba_delba));
                    if (skb == NULL)
                    {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    ...
                    delba->dl_header.ia_category = WIFINET_ACTION_CAT_BA;
                    ...
                    *(unsigned short *)&delba->dl_delbaparamset = htole16(*(unsigned short *)&delbaparamset);
                    ...
                    DPRINTF(AML_DEBUG_ADDBA, "%s: DELBA action mgt frame. TID %d, initiator %d, reason %d\n",
        ...
        case WIFINET_ACTION_CAT_HT:
            switch (action)
            {
                case WIFINET_ACTION_HT_TXCHWIDTH:
                    ...
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_ht_txchwidth));
                    if (skb == NULL)
                    {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    ...
                    txchwidth->at_header.ia_category = WIFINET_ACTION_CAT_HT;
        ...
        case WIFINET_ACTION_CAT_PUBLIC:
            switch (action) {
                case WIFINET_ACT_PUBLIC_BSSCOEXIST:
                    ...
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_bss_coex_frame) + num_channels);
                    if (skb == NULL) {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    }
                    bsscoex = (struct wifi_mac_action_bss_coex_frame *)frm;
                    bsscoex->ac_header.ia_category = WIFINET_ACTION_CAT_PUBLIC;
            ...
            break;
        case WIFINET_ACTION_CAT_WMM:
            switch (action) {
                case WIFINET_ACTION_ADDTS_REQ:
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_addts_req_frame));
                    if (skb == NULL) {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    }
                    addts_req = (struct wifi_mac_action_addts_req_frame *)frm;
                    addts_req->ac_header.ia_category = WIFINET_ACTION_CAT_WMM;
            ...
            case WIFINET_ACTION_DELTS:
                    tid_index = actionargs->arg1;
                    skb = wifi_mac_get_mgmt_frm(wifi_mac, &frm, sizeof(struct wifi_mac_action_delts_req_frame));
                    if (skb == NULL) {
                        wnet_vif->vif_sts.sts_tx_no_buf++;
                        return ENOMEM;
                    }
                    delts_req = (struct wifi_mac_action_delts_req_frame *)frm;
                    delts_req->ac_header.ia_category = WIFINET_ACTION_CAT_WMM;
